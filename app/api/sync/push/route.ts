...
const UPSERTS: Record<SyncTable, string> = {
  exercise_types: `
    INSERT INTO exercise_types
      (id, name, tracks_reps, tracks_duration, created_at, updated_at, deleted_at)
  ...
  exercise_entries: `
    INSERT INTO exercise_entries
      (id, exercise_type_id, reps, duration_seconds, notes, performed_at,
...
function params(table: SyncTable, row: Record<string, unknown>): unknown[] {
  switch (table) {
    case 'exercise_types':
      return [
        row.id, row.name, row.tracks_reps, row.tracks_duration,
        row.created_at, row.updated_at, row.deleted_at,
      ]
    case 'exercise_entries':
      return [
        row.id, row.exercise_type_id, row.reps, row.duration_seconds, row.notes,
